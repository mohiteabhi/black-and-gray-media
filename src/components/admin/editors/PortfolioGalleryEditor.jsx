// src/components/admin/editors/PortfolioGalleryEditor.jsx
...
const CATEGORY_OPTIONS = ["portrait", "abstract", "wedding", "nature", "events", "fashion", "street", "other"];
...
function ItemModal({ item, onSave, onClose, saving }) {
  ...
  const [category,  setCategory]  = useState(item?.category || "portrait");
  ...
  function handleFile(f) {
    ...
    if (!isVideo && !isImage) { setFileError("Unsupported file type."); return; }
    if (f.size > maxBytes) {
      setFileError(`File too large — ${isVideo ? "videos" : "images"} must be under ${maxLabel}. Your file: ${(f.size / 1024 / 1024).toFixed(1)} MB`);
      return;
    }
    setFile(f);
      ...
      <div className="bg-[#0f1423] border border-white/10 rounded-2xl w-full max-w-lg shadow-2xl my-auto">
        ...
        <div className="p-6 flex flex-col gap-5">
          ...
          <div>
            ...
            <div
              className={`relative rounded-xl border-2 border-dashed transition-all overflow-hidden cursor-pointer ${dragging ? "border-blue-500/60 bg-blue-500/5" : "border-white/10 hover:border-white/25 bg-white/[0.02]"}`}
              ...
              {preview ? (
                <div className="w-full h-full relative">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center">
                    <p className="text-white/70 text-xs">Click to change</p>
                  </div>
                </div>
              ) : (
          ...
          <div>
            ...
            <input type="text" value={title} onChange={e => setTitle(e.target.value)}
              placeholder="e.g. Golden Hour Portrait" className={inputCls} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={category} onChange={e => setCategory(e.target.value)} className={inputCls + " cursor-pointer"}>
                {CATEGORY_OPTIONS.map(c => <option key={c} value={c} className="bg-[#0f1423] capitalize">{c}</option>)}
              </select>
            </div>
            <div>
...
export default function PortfolioGalleryEditor() {
  ...
  const displayedItems = filterCat === "all" ? items : items.filter(i => i.category?.toLowerCase() === filterCat);
    ...
    <>
      <div className="flex flex-col gap-6">
          ...
          {categories.map(cat => (
            <button key={cat} onClick={() => setFilterCat(cat)}
              className={`px-3 py-1.5 rounded-lg text-xs capitalize transition-all border ${
                filterCat === cat
                  ? "bg-blue-500/15 border-blue-500/25 text-blue-300 font-semibold"
                  : "border-white/10 text-white/40 hover:text-white/70 hover:border-white/20"
              }`}>
              {cat} {cat !== "all" && `(${items.filter(i => i.category?.toLowerCase() === cat).length})`}
            </button>
          ))}
        </div>
        ...
        {displayedItems.length === 0 ? (
          <p className="text-white/20 text-sm text-center py-8">No items yet. Click "+ Add Item" to start.</p>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/60 transition-all duration-200 opacity-0 group-hover:opacity-100 flex flex-col justify-between p-2">
                  <div className="flex justify-end gap-1.5">
                    <button onClick={() => setModal({ mode: "edit", item })}
                      className="px-2.5 py-1 rounded-lg bg-white/10 hover:bg-white/25 border border-white/20 text-white text-[10px] font-semibold transition-all">Edit</button>
                    <button onClick={() => setDeleteTarget(item)}
                      className="px-2.5 py-1 rounded-lg bg-red-500/20 hover:bg-red-500/40 border border-red-500/30 text-red-300 text-[10px] font-semibold transition-all">✕</button>
                  </div>
