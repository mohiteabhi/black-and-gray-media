// src/components/admin/editors/InfluencerGalleryEditor.jsx
...
const CATEGORY_OPTIONS = ["Teaser", "Cinematic", "Events", "Wedding", "Behind BTS", "Reels", "Brand", "Other"];
...
function ItemModal({ item, onSave, onClose, saving }) {
  ...
  const [category,  setCategory]  = useState(item?.category || "Cinematic");
  ...
  function handleSubmit() {
    ...
    onSave({ file, fields: { page_id: PAGE_ID, section_id: SECTION_ID, title: title.trim(), category, span, sort_order: sortOrder !== "" ? Number(sortOrder) : undefined }, isEdit, id: item?.id });
  }
    ...
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-[#0f1423] border border-white/10 rounded-2xl w-full max-w-lg shadow-2xl my-auto">
        ...
        <div className="p-6 flex flex-col gap-5">
          {/* File dropzone */}
          <div>
            <label className={labelCls}>{isEdit ? "Replace File (optional)" : "File *"}</label>
            <div className={`relative rounded-xl border-2 border-dashed transition-all overflow-hidden cursor-pointer ${dragging ? "border-blue-500/60 bg-blue-500/5" : "border-white/10 hover:border-white/25 bg-white/[0.02]"}`}
          ...
          <div>
            ...
            <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Golden Frame" className={inputCls} />
          </div>
          {/* Category + Span */}
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={span} onChange={e => setSpan(e.target.value)} className={inputCls + " cursor-pointer"}>
                {SPAN_OPTIONS.map(s => <option key={s} value={s} className="bg-[#0f1423]">{s} {s === "large" ? "(2 rows)" : "(1 row)"}</option>)}
              </select>
            </div>
          </div>
          {/* Sort order */}
          <div>
            ...
            <input type="number" value={sortOrder} onChange={e => setSortOrder(e.target.value)} placeholder="1, 2, 3…" className={inputCls} min="0" />
          </div>
        </div>
        <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-white/[0.06]">
          ...
          <button onClick={handleSubmit} disabled={saving || (!isEdit && !file) || Boolean(fileError)}
            className="flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-semibold text-white bg-gradient-to-br from-blue-600 to-indigo-600 shadow-[0_4px_14px_rgba(37,99,235,0.25)] hover:opacity-90 transition-all disabled:opacity-40 disabled:cursor-not-allowed">
...
// ── Main editor ───────────────────────────────────────────────────────────────
export default function InfluencerGalleryEditor() {
  ...
  async function handleSave({ file, fields, isEdit, id }) {
    ...
    try {
      if (isEdit) {
        await patchGalleryItem(id, { title: fields.title, category: fields.category, span: fields.span, sort_order: fields.sort_order });
      ...
      setModal(null);
      await load();
    } catch (err) { setGlobalStatus({ type: "error", message: err.message }); }
    ...
    return (
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <Skeleton className="h-3 w-32" /><Skeleton className="h-9 w-24 rounded-lg" />
        </div>
  ...
  return (
    <>
      <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setModal({ mode: "add" })}
              className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-semibold text-white bg-gradient-to-br from-blue-600 to-indigo-600 shadow-[0_4px_14px_rgba(37,99,235,0.2)] hover:opacity-90 transition-all">
              + Add Item
            </button>
          </div>
        </div>
        ...
        {/* Teaser */}
        <div>
          <p className="text-[10px] font-bold tracking-widest uppercase text-white/25 mb-3">Featured Reel</p>
          {teaser ? (
            <div className="relative group rounded-xl overflow-hidden bg-zinc-900" style={{ aspectRatio: "16/9" }}>
              ...
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/60 transition-all duration-200 flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100">
                <button onClick={() => setModal({ mode: "edit", item: teaser })} className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20 border border-white/20 text-white text-xs font-semibold transition-all">Edit</button>
                <button onClick={() => setDeleteTarget(teaser)} className="px-4 py-2 rounded-lg bg-red-500/20 hover:bg-red-500/40 border border-red-500/30 text-red-300 text-xs font-semibold transition-all">Delete</button>
              </div>
            ...
            <div onClick={() => setModal({ mode: "add" })}
              ...
              <p className="text-white/30 text-sm">Add featured reel</p>
                ...
                <div key={item.id} className="relative group rounded-xl overflow-hidden bg-zinc-900 aspect-square">
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/60 transition-all duration-200 opacity-0 group-hover:opacity-100 flex flex-col justify-between p-2">
                    <div className="flex justify-end gap-1.5">
                      <button onClick={() => setModal({ mode: "edit", item })} className="px-2.5 py-1 rounded-lg bg-white/10 hover:bg-white/25 border border-white/20 text-white text-[10px] font-semibold transition-all">Edit</button>
                      <button onClick={() => setDeleteTarget(item)} className="px-2.5 py-1 rounded-lg bg-red-500/20 hover:bg-red-500/40 border border-red-500/30 text-red-300 text-[10px] font-semibold transition-all">✕</button>
                    </div>
      ...
      {modal && <ItemModal item={modal.item} onSave={handleSave} onClose={() => setModal(null)} saving={saving} />}
      {deleteTarget && (
        <ConfirmModal
